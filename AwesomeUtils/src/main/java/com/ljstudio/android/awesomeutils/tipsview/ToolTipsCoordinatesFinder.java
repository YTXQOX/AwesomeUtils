...
class ToolTipsCoordinatesFinder {
     ...
     * @return point
     */
    static Point getCoordinates(final TextView tipView, ToolTips tooltip) {
        ...
        point.x += UIUtil.isRtl() ? -tooltip.getOffsetX() : tooltip.getOffsetX();
    ...
    private static Point getPositionRightTo(TextView tipView, ToolTips toolTips, Coordinates anchorViewCoordinates, Coordinates rootLocation) {
        Point point = new Point();
        point.x = anchorViewCoordinates.right;
        AdjustRightToOutOfBounds(tipView, toolTips.getRootView(), point, anchorViewCoordinates, rootLocation);
        point.y = anchorViewCoordinates.top + getYCenteringOffset(tipView, toolTips);
        return point;
    }

    private static Point getPositionLeftTo(TextView tipView, ToolTips toolTips, Coordinates anchorViewCoordinates, Coordinates rootLocation) {
        Point point = new Point();
        point.x = anchorViewCoordinates.left - tipView.getMeasuredWidth();
        AdjustLeftToOutOfBounds(tipView, toolTips.getRootView(), point, anchorViewCoordinates, rootLocation);
        point.y = anchorViewCoordinates.top + getYCenteringOffset(tipView, toolTips);
        return point;
    }

    private static Point getPositionBelow(TextView tipView, ToolTips toolTips, Coordinates anchorViewCoordinates, Coordinates rootLocation) {
        Point point = new Point();
        point.x = anchorViewCoordinates.left + getXOffset(tipView, toolTips);
        if (toolTips.alignedCenter()) {
            AdjustHorizontalCenteredOutOfBounds(tipView, toolTips.getRootView(), point, rootLocation);
        } else if (toolTips.alignedLeft()){
            AdjustHorizontalLeftAlignmentOutOfBounds(tipView, toolTips.getRootView(), point, anchorViewCoordinates, rootLocation);
        } else if (toolTips.alignedRight()){
            AdjustHorizotalRightAlignmentOutOfBounds(tipView, toolTips.getRootView(), point, anchorViewCoordinates, rootLocation);
        }
        point.y = anchorViewCoordinates.bottom;
        return point;
    }

    private static Point getPositionAbove(TextView tipView, ToolTips toolTips,
     ...
     * @return int
     */
    private static int getXOffset(View tipView, ToolTips toolTips) {
        int offset;

        switch (toolTips.getAlign()) {
            case ToolTips.ALIGN_CENTER:
                offset = ((toolTips.getAnchorView().getWidth() - tipView.getMeasuredWidth()) / 2);
                break;
            case ToolTips.ALIGN_LEFT:
                offset = 0;
                break;
            case ToolTips.ALIGN_RIGHT:
                offset = toolTips.getAnchorView().getWidth() - tipView.getMeasuredWidth();
                break;
            default:
                offset = 0;
                break;
     ...
     * @return int
     */
    private static int getYCenteringOffset(View tipView, ToolTips toolTips) {
        return (toolTips.getAnchorView().getHeight() - tipView.getMeasuredHeight()) / 2;
    }
